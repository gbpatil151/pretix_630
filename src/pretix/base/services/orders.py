...
class _OrderChangeOperationsRunner:
    """
    Subsystem responsible for applying queued order-change operations during commit.

    ``OrderChangeManager`` acts as a Facade: it exposes the public change API and
    delegates the mutation/commit pipeline to this class to separate concerns.
    """

    __slots__ = ('_m',)

    def __init__(self, manager: 'OrderChangeManager'):
        self._m = manager

    def run(self):
        nextposid = self._m.order.all_positions.aggregate(m=Max('positionid'))['m'] + 1
        split_positions = []
        secret_dirty = set()
        position_cache = {}
        fee_cache = {}

        for op in self._m._operations:
            if isinstance(op, self._m.ItemOperation):
                self._perform_order_change_item_operation(op, position_cache, secret_dirty)
            elif isinstance(op, self._m.MembershipOperation):
                self._perform_order_change_membership_operation(op, position_cache)
            elif isinstance(op, self._m.SeatOperation):
                self._perform_order_change_seat_operation(op, position_cache, secret_dirty)
            elif isinstance(op, self._m.SubeventOperation):
                self._perform_order_change_subevent_operation(op, position_cache, secret_dirty)
            elif isinstance(op, self._m.AddFeeOperation):
                self._perform_order_change_add_fee_operation(op)
            elif isinstance(op, self._m.FeeValueOperation):
                self._perform_order_change_fee_value_operation(op, fee_cache)
            elif isinstance(op, self._m.PriceOperation):
                self._perform_order_change_price_operation(op, position_cache)
            elif isinstance(op, self._m.TaxRuleOperation):
                self._perform_order_change_tax_rule_operation(op, position_cache, fee_cache)
            elif isinstance(op, self._m.CancelFeeOperation):
                self._perform_order_change_cancel_fee_operation(op, fee_cache)
            elif isinstance(op, self._m.CancelOperation):
                self._perform_order_change_cancel_operation(op, position_cache, secret_dirty)
            elif isinstance(op, self._m.AddOperation):
                nextposid = self._perform_order_change_add_operation(op, nextposid)
            elif isinstance(op, self._m.SplitOperation):
                self._perform_order_change_split_operation(op, position_cache, split_positions)
            elif isinstance(op, self._m.RegenerateSecretOperation):
                self._perform_order_change_regenerate_secret_operation(op, position_cache, secret_dirty)
            elif isinstance(op, self._m.ChangeSecretOperation):
                self._perform_order_change_change_secret_operation(op, secret_dirty)
            elif isinstance(op, self._m.ChangeValidFromOperation):
                self._perform_order_change_valid_from_operation(op, position_cache, secret_dirty)
            elif isinstance(op, self._m.ChangeValidUntilOperation):
                self._perform_order_change_valid_until_operation(op, position_cache, secret_dirty)
            elif isinstance(op, self._m.AddBlockOperation):
                self._perform_order_change_add_block_operation(op, position_cache)
            elif isinstance(op, self._m.RemoveBlockOperation):
                self._perform_order_change_remove_block_operation(op, position_cache)
            elif isinstance(op, self._m.ForceRecomputeOperation):
        ...
        for p in secret_dirty:
            assign_ticket_secret(
                event=self._m.event, position=p, force_invalidate=False, save=True
            )

        if split_positions:
            self._m.split_order = self._create_split_order(split_positions)
    ...
    def _perform_order_change_item_operation(self, op, position_cache, secret_dirty):
        ...
        self._m.order.log_action('pretix.event.order.changed.item', user=self._m.user, auth=self._m.auth, data={
    ...
    def _perform_order_change_membership_operation(self, op, position_cache):
        position = position_cache.setdefault(op.position.pk, op.position)
        self._m.order.log_action('pretix.event.order.changed.membership', user=self._m.user, auth=self._m.auth, data={
            'position': position.pk,
            'positionid': position.positionid,
    ...
    def _perform_order_change_seat_operation(self, op, position_cache, secret_dirty):
        position = position_cache.setdefault(op.position.pk, op.position)
        self._m.order.log_action('pretix.event.order.changed.seat', user=self._m.user, auth=self._m.auth, data={
            'position': position.pk,
            'positionid': position.positionid,
    ...
    def _perform_order_change_subevent_operation(self, op, position_cache, secret_dirty):
        position = position_cache.setdefault(op.position.pk, op.position)
        self._m.order.log_action('pretix.event.order.changed.subevent', user=self._m.user, auth=self._m.auth, data={
            'position': position.pk,
            'positionid': position.positionid,
    ...
    def _perform_order_change_add_fee_operation(self, op):
        self._m.order.log_action('pretix.event.order.changed.addfee', user=self._m.user, auth=self._m.auth, data={
            'fee': op.fee.pk,
        })
        op.fee.order = self._m.order
    ...
    def _perform_order_change_fee_value_operation(self, op, fee_cache):
        ...
        self._m.order.log_action('pretix.event.order.changed.feevalue', user=self._m.user, auth=self._m.auth, data={
            'fee': fee.pk,
            'old_price': fee.value,
    ...
    def _perform_order_change_price_operation(self, op, position_cache):
        position = position_cache.setdefault(op.position.pk, op.position)
        self._m.order.log_action('pretix.event.order.changed.price', user=self._m.user, auth=self._m.auth, data={
            'position': position.pk,
            'positionid': position.positionid,
            'old_price': position.price,
            'addon_to': position.addon_to_id,
    ...
    def _perform_order_change_tax_rule_operation(self, op, position_cache, fee_cache):
        if isinstance(op.position, OrderPosition):
            position = position_cache.setdefault(op.position.pk, op.position)
            self._m.order.log_action('pretix.event.order.changed.tax_rule', user=self._m.user, auth=self._m.auth, data={
                'position': position.pk,
                'positionid': position.positionid,
                'addon_to': position.addon_to_id,
    ...
    def _perform_order_change_cancel_fee_operation(self, op, fee_cache):
        fee = fee_cache.setdefault(op.fee.pk, op.fee)
        self._m.order.log_action('pretix.event.order.changed.cancelfee', user=self._m.user, auth=self._m.auth, data={
            'fee': fee.pk,
            'fee_type': fee.fee_type,
            'old_price': fee.value,
        })
    ...
    def _perform_order_change_cancel_operation(self, op, position_cache, secret_dirty):
        ...
        for opa in position.addons.all():
            ...
            self._m.order.log_action('pretix.event.order.changed.cancel', user=self._m.user, auth=self._m.auth, data={
                'position': opa.pk,
            ...
            assign_ticket_secret(
                event=self._m.event, position=opa, force_invalidate_if_revokation_list_used=True, force_invalidate=False, save=False
            )
        ...
        if position.voucher:
            Voucher.objects.filter(pk=position.voucher.pk).update(redeemed=Greatest(0, F('redeemed') - 1))
        assign_ticket_secret(
            event=self._m.event, position=position, force_invalidate_if_revokation_list_used=True, force_invalidate=False, save=False
        )
    ...
    def _perform_order_change_add_operation(self, op, nextposid):
        pos = OrderPosition.objects.create(
            ...
            price=op.price.gross, order=self._m.order, tax_rate=op.price.rate, tax_code=op.price.code,
        ...
        nextposid += 1
        self._m.order.log_action('pretix.event.order.changed.add', user=self._m.user, auth=self._m.auth, data={
            'position': pos.pk,
            'item': op.item.pk,
    ...
    def _perform_order_change_regenerate_secret_operation(self, op, position_cache, secret_dirty):
        ...
        assign_ticket_secret(
            event=self._m.event, position=position, force_invalidate=True, save=True
        )
        if position in secret_dirty:
            secret_dirty.remove(position)
        tickets.invalidate_cache.apply_async(kwargs={'event': self._m.event.pk,
                                                     'order': self._m.order.pk})
        self._m.order.log_action('pretix.event.order.changed.secret', user=self._m.user, auth=self._m.auth, data={
            'position': position.pk,
            'positionid': position.positionid,
        })

    def _perform_order_change_change_secret_operation(self, op, secret_dirty):
        if OrderPosition.all.filter(order__event=self._m.event, secret=op.new_secret).exists():
    ...
    def _perform_order_change_valid_from_operation(self, op, position_cache, secret_dirty):
        position = position_cache.setdefault(op.position.pk, op.position)
        self._m.order.log_action('pretix.event.order.changed.valid_from', user=self._m.user, auth=self._m.auth, data={
            'position': position.pk,
            'positionid': position.positionid,
    ...
    def _perform_order_change_valid_until_operation(self, op, position_cache, secret_dirty):
        position = position_cache.setdefault(op.position.pk, op.position)
        self._m.order.log_action('pretix.event.order.changed.valid_until', user=self._m.user, auth=self._m.auth, data={
            'position': position.pk,
            'positionid': position.positionid,
    ...
    def _perform_order_change_add_block_operation(self, op, position_cache):
        position = position_cache.setdefault(op.position.pk, op.position)
        self._m.order.log_action('pretix.event.order.changed.add_block', user=self._m.user, auth=self._m.auth, data={
            'position': position.pk,
            'positionid': position.positionid,
    ...
    def _perform_order_change_remove_block_operation(self, op, position_cache):
        position = position_cache.setdefault(op.position.pk, op.position)
        self._m.order.log_action('pretix.event.order.changed.remove_block', user=self._m.user, auth=self._m.auth, data={
            'position': position.pk,
            'positionid': position.positionid,
            'block_name': op.block_name,
        })
    ...
    def _perform_order_change_force_recompute_operation(self):
        self._m.order.log_action('pretix.event.order.changed.recomputed', user=self._m.user, auth=self._m.auth, data={})

    def _create_split_order(self, split_positions):
        split_order = Order.objects.get(pk=self._m.order.pk)
        split_order.pk = None
        split_order.code = None
        split_order.datetime = now()
        split_order.secret = generate_secret()
        split_order.require_approval = self._m.order.require_approval and any(p.requires_approval(invoice_address=self._m._invoice_address) for p in split_positions)
        split_order.save()
        split_order.log_action('pretix.event.order.changed.split_from', user=self._m.user, auth=self._m.auth, data={
            'original_order': self._m.order.code
        })

        for op in split_positions:
            self._m.order.log_action('pretix.event.order.changed.split', user=self._m.user, auth=self._m.auth, data={
                'position': op.pk,
            ...
            assign_ticket_secret(
                self._m.event, position=op, force_invalidate=True,
            )
            op.save()

        try:
            ia = modelcopy(self._m.order.invoice_address)
            ia.pk = None
        ...
        fees = []
        for fee in self._m.order.fees.exclude(fee_type=OrderFee.FEE_TYPE_PAYMENT):
        ...
        changed_by_rounding = set(apply_rounding(
            self._m.order.tax_rounding_mode,
            self._m._invoice_address,
            self._m.event.currency,
        ...
        if split_order.total != Decimal('0.00') and self._m.order.status != Order.STATUS_PAID:
            pp = self._m._get_payment_provider()
            if pp:
        ...
        remaining_total = sum([p.price for p in self._m.order.positions.all()]) + sum([f.value for f in self._m.order.fees.all()])
        offset_amount = min(max(0, self._m.completed_payment_sum - remaining_total), split_order.total)
        if offset_amount >= split_order.total and not split_order.require_approval:
            ...
            if self._m.order.status == Order.STATUS_PAID:
        ...
        if offset_amount > Decimal('0.00'):
            split_order.payments.create(
                ...
                payment_date=now(),
                provider='offsetting',
                info=json.dumps({'orders': [self._m.order.code]})
            )
            self._m.order.refunds.create(
        ...
        if split_order.total != Decimal('0.00') and self._m.order.invoices.filter(is_cancellation=False).last():
            try:
        ...
        order_split.send(sender=self._m.order.event, original=self._m.order, split_order=split_order)
        return split_order
...
class OrderChangeManager:
    ...
    def __init__(self, order: Order, user=None, auth=None, notify=True, reissue_invoice=True, allow_blocked_seats=False):
        ...
        self._invoices = []
        self._operations_runner = _OrderChangeOperationsRunner(self)
