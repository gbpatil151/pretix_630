...
class IndexView(EventPermissionRequiredMixin, ChartContainingView, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Payment-date subqueries shared by several chart builders
        p_date = self._payment_date_subquery(OuterRef('pk'))
        op_date = self._payment_date_subquery(OuterRef('order'))

        # Delegate each chart section to its own helper
        self._build_orders_by_day_data(ctx, cache, ckey, subevent, tz, p_date)
        self._build_attendees_by_day_data(ctx, cache, ckey, subevent, tz, op_date)
        self._build_orders_by_product_data(ctx, cache, ckey, subevent)
        self._build_revenue_data(ctx, cache, ckey, subevent, tz, p_date, op_date)

        ctx['has_orders'] = self.request.event.orders.exists()
        ctx['seats'] = {}
        self._build_seats_data(ctx, ckey, subevent)
    ...
    @staticmethod
    def _payment_date_subquery(outer_ref):
        """Return a Subquery that resolves to the latest confirmed/refunded payment date."""
    ...
    @staticmethod
    def _date_range(ordered_by_day, paid_by_day=None):
        """Compute the (start, end) date-range for a daily rrule."""
        if not ordered_by_day:
            return datetime.date.today(), datetime.date.today()
        start = min(ordered_by_day.keys())
        end_candidates = [
            max(ordered_by_day.keys() if paid_by_day else [datetime.date.today()]),
        ]
        if paid_by_day:
            end_candidates.append(max(paid_by_day.keys()))
        else:
            end_candidates.append(datetime.date(1970, 1, 1))
        return start, max(end_candidates)

    # ------------------------------------------------------------------
    # Chart builders — each handles cache-check → query → transform → set
    # ------------------------------------------------------------------

    def _build_orders_by_day_data(self, ctx, cache, ckey, subevent, tz, p_date):
        ...
        start, end = self._date_range(ordered_by_day, paid_by_day)
        data = []
    ...
    def _build_attendees_by_day_data(self, ctx, cache, ckey, subevent, tz, op_date):
        """Attendees-by-day + cumulative-over-time charts."""
        ctx['abd_data'] = cache.get('statistics_abd_data' + ckey)
        ctx['abt_data'] = cache.get('statistics_abt_data' + ckey)
        if ctx['abd_data'] and ctx['abt_data']:
            return
    ...
    def _build_orders_by_product_data(self, ctx, cache, ckey, subevent):
    ...
    def _build_revenue_data(self, ctx, cache, ckey, subevent, tz, p_date, op_date):
        ...
        data = []
        total = 0
        start = min(rev_by_day.keys()) if rev_by_day else datetime.date.today()
        end = max(rev_by_day.keys()) if rev_by_day else datetime.date.today()
        for d in dateutil.rrule.rrule(dateutil.rrule.DAILY, dtstart=start, until=end):
            d = d.date()
    ...
    def _build_seats_data(self, ctx, ckey, subevent):
        """Seating plan statistics (only for single-event or specific subevent)."""
        if self.request.event.has_subevents and not (ckey != "all" and subevent):
            return

        ev = subevent or self.request.event
        if ev.seating_plan_id is None:
            return
        ...
        for item in seats_qs:
            ...
            if product not in ctx['seats']['products']:
                price = self._resolve_seat_price(product)
                ctx['seats']['products'][product] = {
                    'free': {'seats': 0, 'potential': Decimal('0.00')},
                    'blocked': {'seats': 0, 'potential': Decimal('0.00')},
                    'price': price,
                }
    ...
    @staticmethod
    def _resolve_seat_price(product):
        """Determine the display price for a seat product."""
        price = None
