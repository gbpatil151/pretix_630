...
class OrderCreateSerializer(I18nAwareModelSerializer):
    ...
    def create(self, validated_data):
        ...
        Each phase is a focused, independently-testable method on OrderBuilder.
        See issue #61 for rationale.
        """
        from pretix.api.serializers.order_builder import OrderBuilder

        builder = OrderBuilder(self.context, validated_data)

        # Phase 1 – extract nested data from validated_data
        builder.extract_data()

        # Phase 2 – compute quota / voucher / seat diffs
        builder.build_resource_diffs()

        # Phase 3 – lock resources and validate availability
        builder.lock_and_validate()

        # Phase 4 – create Order + OrderPosition objects
        builder.create_order_and_positions()

        # Phase 5 – create fees, apply tax rounding
        builder.attach_fees()

        # Phase 6 – handle payments, return the order
        order = builder.process_payments()

        # Preserve side-effect used by the view layer
